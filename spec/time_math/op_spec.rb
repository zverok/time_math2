  ...
  describe 'default' do
    ...
    context '#==' do
      ...
      it 'is not equal on different ops or params' do
        expect(described_class.new.floor(:month).advance(:day, 3))
          .to_not eq described_class.new.ceil(:month).advance(:day, 3)

        expect(described_class.new.floor(:month).advance(:day, 3))
          .to_not eq described_class.new.floor(:month).advance(:day, 2)
      end

      it 'considers op order' do
        expect(described_class.new.floor(:month).advance(:day, 3))
          .to_not eq described_class.new.advance(:day, 3).floor(:month)
      end

      it 'considers arguments' do
        expect(described_class.new.floor(:month).advance(:day, 3))
          .to_not eq described_class.new(tm).floor(:month).advance(:day, 3)
      end
    end
